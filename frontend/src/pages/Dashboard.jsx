...
export default function Dashboard() {
  const {
    data: teamsData, loading: teamsLoading, error: teamsError, refetch: refetchTeams,
  } = useFetch(() => teamsAPI.getAll({ per_page: 83 }))

  const { data: scorersData, loading: scorersLoading } =
    useFetch(() => playersAPI.getTopScorers(10))

  const { data: avgData, loading: avgLoading } =
    ...
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        {avgLoading || teamsLoading ? (
          ...
          <>
            <StatCard icon="Total matches" label="Total Matches"
              value={avgs.total_matches?.toLocaleString() || '—'}
              sub="World Cup history" />
            <StatCard icon="Goals per match" label="Avg Goals / Match"
              value={avgs.avg_goals_per_match || '—'}
              sub={`xG avg: ${avgs.avg_xg || '—'}`} />
            <StatCard icon="Nations" label="Nations"
              value={teams.length}
              sub={`Most: ${topConf}`} />
            <StatCard icon="Shots per match" label="Avg Shots / Match"
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          {teamsLoading ? (
            <div className="card">
              <div className="px-5 py-4 border-b border-border">
                <div className="h-5 bg-surface2 w-24 animate-pulse" />
              </div>
        ...
        <div>
          {scorersLoading ? (
            <div className="card">
              <div className="px-5 py-4 border-b border-border">
                <div className="h-5 bg-surface2 w-28 animate-pulse mb-1" />
                <div className="h-3 bg-surface2 w-40 animate-pulse" />
              </div>
